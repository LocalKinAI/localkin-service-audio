...
def _registry_models_as_dicts() -> List[Dict[str, Any]]:
    """Convert model registry entries to legacy dict format for templates."""
    result = []
        ...
        if "kokoro" in model_name.lower():
            ...
            try:
                ...
                if audio_data:
                    ...
                    print(f"✅ Kokoro TTS synthesis complete with voice: {selected_voice}")
                    return True
                else:
            ...
            try:
                ...
                try:
                    ...
                    print("✅ XTTS v2 synthesis complete")
                    return True
            ...
            # Unsupported model for local synthesis
            print(f"❌ Local synthesis not supported for {model_name}")
            return False

    except Exception as e:
        print(f"❌ HuggingFace TTS with voice failed: {e}")
        return False
...
async def home(request: Request):
    ...
    models = _registry_models_as_dicts()
...
async def transcribe_page(request: Request):
    ...
    models = [m for m in _registry_models_as_dicts() if m.get("type") == "stt"]
...
async def synthesize_page(request: Request):
    ...
    models = [m for m in _registry_models_as_dicts() if m.get("type") == "tts"]
...
async def api_transcribe(
    ...
    try:
        ...
        # Use AudioEngine for transcription
        from ..core.audio_processing import AudioEngine
        engine = AudioEngine()

        # Map model name to engine:size format
        if model_name.startswith("faster-whisper"):
            engine_model = f"faster-whisper:{model_name.replace('faster-whisper-', '')}"
        elif model_name.startswith("whisper-cpp"):
            engine_model = f"whisper-cpp:{model_name.replace('whisper-cpp-', 'base')}"
        elif model_name.startswith("whisper-"):
            engine_model = f"whisper:{model_name.replace('whisper-', '')}"
        elif model_name == "whisper":
            engine_model = "whisper:base"
        else:
            engine_model = model_name

        engine.load_stt(engine_model)
        result = engine.transcribe(str(file_path))
    ...
    try:
        ...
        if not api_success:
            ...
            if model_name == 'native':
                # Use native TTS locally via AudioEngine
                from ..core.audio_processing import AudioEngine
                engine = AudioEngine()
                engine.load_tts("native")
                engine.synthesize(synthesis_text, str(output_path))
